...
exports.createTourZodSchema = zod_1.z.object({
    ...
    tourType: zod_1.z
        .string({
        // Assuming tourType is a string (ObjectId string)
    ...
    division: zod_1.z
        .string({
        // Assuming division is a string (ObjectId string)
    ...
    costForm: zod_1.z
        .number({
        // Match Mongoose schema: costForm, type Number
        invalid_type_error: "Cost must be a number",
    })
        .optional(),
    images: zod_1.z.array(zod_1.z.string()).optional(), // Match Mongoose schema: images, type Array of String
    startDate: zod_1.z.string({ message: "Start date must be a valid date string" }).refine((val) => !isNaN(Date.parse(val)), {
        message: "Start date must be a valid date",
    }),
    endDate: zod_1.z
        .string({ message: "End date must be a valid date string" }).refine((val) => !isNaN(Date.parse(val)), {
        message: "Start date must be a valid date",
    }),
    ...
    maxGuest: zod_1.z
        ...
        .positive("Max guest must be a positive number")
        .optional(),
    minAge: zod_1.z
        .number({
...
exports.updateTourZodSchema = zod_1.z.object({
    ...
    startDate: zod_1.z
        .string()
        .datetime({ message: "Start date must be a valid date string" })
        .optional(),
    endDate: zod_1.z
        .string()
        .datetime({ message: "End date must be a valid date string" })
        .optional(),
    departureLocation: zod_1.z.string().optional(),
    arrivalLocation: zod_1.z.string().optional(),
    included: zod_1.z.array(zod_1.z.string()).optional(),
